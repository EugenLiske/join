...
import { loadAllContacts, contactsData, showToastMessage } from './contacts_list_data.js';
import { selectContactById, getCurrentSelectedContact } from './contacts_list_selection.js';
...
export async function openAddContactOverlay() {
    try {
        const html = await loadOverlayHTML('../overlays/contacts/contacts_add.html');
...
export async function deleteSelectedContact() {
    ...
    try {
        ...
        const response = await fetch(`${FIREBASE_URL}${FIREBASE_PATHS.SINGLE_CONTACT(currentContact.id)}`, {
            method: 'DELETE'
        });
